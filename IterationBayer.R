iteration2sls <- function(dataind, data, datatot, formiv, formols, formrqinst1, formrqinst2, formrq, youth = TRUE, initvalue=0){
  ...
  # We need the following two to calculate figures and keep track of representative postcode with correct dimensions
  ...
  instr <- datahat$instrument*100
  instrinter <- datahat$instrument*100*datahat$addrdens
  correlationfirsteq <- lm(datahat$pfield~instr)
  ...
  return(list("iv"= iv,"phi"=phimat, "instrument"=instrumentmat, 
              "alpha_est"=datahat$alpha,"olsout"=ols, "rqout"= rqoutput, "correq"=correlationfirsteq))
}